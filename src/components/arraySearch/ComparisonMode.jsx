...
const SORTED_ARRAY = [12, 17, 23, 30, 37, 45, 50, 63, 72, 88, 90, 99, 101, 120, 160]
...
function SearchPanel({ algoKey, array, target, speed, trigger, onComplete, isWinner }) {
  ...
  useEffect(() => {
    ...
    const generated = algoKey === 'linearSearch'
  ...
  const getElemStyle = (idx) => {
    if (foundIndex === idx) return { background: '#10b981', color: '#fff', borderColor: '#34d399', transform: 'scale(1.15)', boxShadow: '0 0 15px rgba(16,185,129,0.5)' }
    if (algoKey === 'binarySearch' && currentStep) {
      const [mid, low, high] = activeIndices
      if (idx === mid) return { background: '#facc15', color: '#0f172a', borderColor: '#fde047', transform: 'scale(1.1)', boxShadow: '0 0 12px rgba(250,204,21,0.4)' }
      if (idx === low) return { background: '#06b6d4', color: '#fff', borderColor: '#22d3ee' }
      if (idx === high) return { background: '#f43f5e', color: '#fff', borderColor: '#fb7185' }
      if (idx < low || idx > high) return { opacity: 0.25, transform: 'scale(0.9)' }
    }
    if (algoKey === 'linearSearch' && activeIndices.includes(idx)) {
      return { background: meta.accent, color: '#fff', borderColor: meta.accent, transform: 'scale(1.1)', boxShadow: `0 0 12px ${meta.glow}` }
    }
    return {}
  }
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700/50"
        ...
        <span className="text-[10px] font-medium px-2 py-0.5 rounded-full"
          ...
          {isFinished ? (foundIndex !== null ? `Found at [${foundIndex}]` : 'Not Found') : isPlaying ? 'Searching…' : 'Ready'}
        </span>
      </div>
      ...
      <div className="px-4 pb-3">
        <div className="rounded-lg px-3 py-2 min-h-[32px]"
          style={{ background: 'rgba(2,6,23,0.5)', border: '1px solid rgba(30,41,59,0.6)' }}>
          <p className="text-[10px] text-slate-400 leading-relaxed">
            {currentStep?.message || (isFinished ? (foundIndex !== null ? `✓ Found ${target} at index ${foundIndex}` : `✗ ${target} not found`) : 'Waiting to start…')}
          </p>
        </div>
      </div>
      ...
      <div className="px-4 pb-4 grid grid-cols-4 gap-2">
        {[
          { label: 'Comparisons', value: isFinished ? totalComparisons : comparisons },
          ...
          <div key={label} className="rounded-lg p-2" style={{ background: 'rgba(2,6,23,0.7)', border: '1px solid rgba(30,41,59,0.8)' }}>
            <p className="text-[8px] uppercase tracking-widest text-slate-500 mb-0.5 leading-tight">{label}</p>
            <p className="font-mono text-[11px] font-bold leading-tight" style={{ color: meta.accent }}>{value}</p>
          </div>
        ))}
      </div>
    </motion.div>
...
export default function ArraySearchComparison() {
  ...
  return (
    <div className="flex flex-col gap-4 p-4 sm:p-6">
      ...
      <div className="rounded-2xl border border-slate-700/60 p-4" style={{ background: 'rgba(15,23,42,0.8)' }}>
        <div className="flex flex-wrap gap-4 items-end justify-between">
          <div className="flex flex-wrap gap-4 items-end">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-cyan-400/80 mb-2">Target Value</p>
              <div className="flex gap-2 items-center">
                <input type="number" value={target} onChange={(e) => { setTarget(e.target.value); handleReset() }}
                ...
                <div className="flex flex-wrap gap-1">
                  {[17, 37, 72, 99, 120, 999].map(v => (
                    <button key={v} onClick={() => { setTarget(String(v)); handleReset() }}
                      className="px-2 py-1 rounded-lg text-[10px] font-bold border transition-all"
                      style={{
                        background: parseInt(target) === v ? 'rgba(6,182,212,0.15)' : 'rgba(30,41,59,0.5)',
                        borderColor: parseInt(target) === v ? '#06b6d4' : 'rgba(51,65,85,0.8)',
      ...
      <div className="rounded-xl px-4 py-2.5 border border-slate-700/40" style={{ background: 'rgba(15,23,42,0.6)' }}>
        <p className="text-[9px] uppercase tracking-widest text-slate-500 mb-1.5">Shared Sorted Array (both algorithms use this)</p>
        <div className="flex flex-wrap gap-1">
          {SORTED_ARRAY.map((v, i) => (
            <span key={i} className="font-mono text-[10px] font-bold px-1.5 py-0.5 rounded"
              style={{
                background: parseInt(target) === v ? 'rgba(16,185,129,0.2)' : 'rgba(30,41,59,0.5)',
          ...
          <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400">
                Binary Search wins on sorted data — O(log n) vs O(n) makes a real difference.
              </p>
            </div>
          </motion.div>
        )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {Object.keys(ALGO_META).map(key => (
          <SearchPanel key={key} algoKey={key} array={SORTED_ARRAY} target={target}
            speed={speed} trigger={trigger} onComplete={handleComplete} isWinner={winner === key} />
        ))}
      </div>

      <div className="rounded-xl px-4 py-3 text-xs text-slate-400 border border-slate-700/40"
        style={{ background: 'rgba(15,23,42,0.6)' }}>
        <strong className="text-slate-300">Key insight:</strong> Binary search requires a <em>sorted</em> array. It's dramatically faster for large datasets, but linear search works on any array — unsorted or sorted. Try target 999 (not in array) to see how many comparisons each needs.
      </div>
    </div>
  )
}