...
function runDijkstra(source) {
  ...
  const steps = [{ dist: { ...dist }, visited: new Set(visited), current: null, relaxed: [], phase: `Init: dist[${source}] = 0` }]
  ...
  for (let i = 0; i < NODES.length; i++) {
    ...
    steps.push({ dist: { ...dist }, visited: new Set(visited), current: u, relaxed, phase: `Visit node ${u}, relax neighbors` })
  }

  steps.push({ dist: { ...dist }, visited: new Set(visited), current: null, relaxed: [], phase: 'Complete', done: true })
  return steps
}
...
function runBellmanFord(source) {
  ...
  for (const n of NODES) dist[n] = INF
  dist[source] = 0
  const steps = [{ dist: { ...dist }, iteration: 0, relaxed: [], phase: `Init: dist[${source}] = 0` }]
  ...
  for (let i = 1; i < NODES.length; i++) {
    ...
    steps.push({ dist: { ...dist }, iteration: i, relaxed: relaxed.map(r => r.to), phase: `Iteration ${i}: relax all edges` })
    if (relaxed.length === 0) {
      steps.push({ dist: { ...dist }, iteration: i, relaxed: [], phase: 'Early termination — no updates', done: true })
      return steps
    }
  }

  steps.push({ dist: { ...dist }, iteration: NODES.length - 1, relaxed: [], phase: 'Complete', done: true })
  return steps
}
...
function runFloydWarshall() {
  ...
  const steps = [{ dist: dist.map(r => [...r]), k: null, phase: 'Initialize distance matrix' }]
  ...
  for (let k = 0; k < n; k++) {
    ...
    steps.push({ dist: dist.map(r => [...r]), k: k + 1, updated, phase: `Intermediate node k=${k + 1}: ${updated.length} paths improved` })
  }

  steps.push({ dist: dist.map(r => [...r]), k: null, updated: [], phase: 'All-pairs shortest paths complete', done: true })
  return steps
}
...
const ALGO_META = {
  dijkstra: {
    ...
    renderStep: (step, meta) => (
      <DijkstraView step={step} meta={meta} />
    ),
  },
  bellmanford: {
    label: 'Bellman-Ford',
    ...
    description: 'Single-source, handles negative weights & detects negative cycles',
    runs: (src) => runBellmanFord(src),
    renderStep: (step, meta) => (
      <BellmanFordView step={step} meta={meta} />
    ),
  },
  floydwarshall: {
...
function DistBar({ node, dist, meta, highlighted }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="font-mono text-[10px] font-bold w-4 text-slate-400">{node}</span>
      <div className="flex-1 h-4 rounded-full overflow-hidden bg-slate-800">
        {val !== INF && (
          <motion.div className="h-full rounded-full"
            style={{ background: highlighted ? meta.accent : meta.accentBorder, width: `${Math.min(100, (val / 20) * 100)}%` }}
            animate={{ width: `${Math.min(100, (val / 20) * 100)}%` }}
            transition={{ duration: 0.3 }} />
        )}
      </div>
      <span className="font-mono text-[10px] font-bold w-10 text-right" style={{ color: val === INF ? '#475569' : highlighted ? meta.accent : '#94a3b8' }}>
        {val === INF ? '∞' : val}
      </span>
    </div>
  )
...
function DijkstraView({ step, meta }) {
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-[9px] uppercase tracking-widest text-slate-500 mb-2">Shortest Distances from Source</p>
      {NODES.map(n => (
        <DistBar key={n} node={n} dist={step.dist} meta={meta}
          highlighted={step.relaxed?.includes(n) || step.current === n} />
      ))}
      <p className="text-[9px] text-slate-500 mt-2">Visited: {step.visited ? [...step.visited].join(', ') || '—' : '—'}</p>
    </div>
  )
}
...
function BellmanFordView({ step, meta }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      <p className="text-[9px] text-slate-500 mt-2">Iteration: {step.iteration} / {NODES.length - 1}</p>
    </div>
  )
}
...
function FloydView({ step, meta }) {
  ...
  return (
    <div>
      <p className="text-[9px] uppercase tracking-widest text-slate-500 mb-2">Distance Matrix (rows=from, cols=to)</p>
      <div className="overflow-x-auto">
        <table className="text-[9px] font-mono border-collapse">
          <thead>
            <tr>
              <td className="w-5" />
              {NODES.map(c => <th key={c} className="w-7 text-center text-slate-500 pb-1">{c}</th>)}
            </tr>
          </thead>
          <tbody>
            {step.dist.map((row, i) => (
              <tr key={i}>
                ...
                {row.map((val, j) => {
                  const isUpdated = step.updated?.some(([ui, uj]) => ui === i + 1 && uj === j + 1)
                  return (
                    <td key={j} className="w-7 h-5 text-center rounded"
                      style={{ color: val === INF ? '#374151' : isUpdated ? meta.accent : '#94a3b8', background: isUpdated ? meta.accentBg : 'transparent' }}>
                      {val === INF ? '∞' : val}
                    </td>
                  )
                })}
...
function AlgoPanel({ algoKey, source, speed, trigger, onComplete, isWinner }) {
  ...
  const stepIdxRef = React.useRef(-1)

  React.useEffect(() => {
    if (trigger === 0) return
    clearTimeout(timeoutRef.current)
    const generated = meta.runs(source)
  ...
  return (
    <motion.div
      ...
      style={{
        background: isWinner ? `linear-gradient(135deg, rgba(15,23,42,0.95), ${meta.accentBg})` : 'rgba(15,23,42,0.85)',
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700/50"
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div>
            ...
            <p className="text-[9px] text-slate-500 max-w-[200px]">{meta.description}</p>
          </div>
          {isWinner && (
      ...
      <div className="px-4 pt-3 pb-1">
        <div className="rounded-lg px-3 py-1.5" style={{ background: 'rgba(2,6,23,0.5)', border: '1px solid rgba(30,41,59,0.6)' }}>
      ...
      <div className="px-4 pb-4 grid grid-cols-3 gap-2">
        {[
          ...
          { label: 'Complexity', value: complexity?.time?.split(',')[0] ?? '—' },
        ].map(({ label, value }) => (
          <div key={label} className="rounded-lg p-2" style={{ background: 'rgba(2,6,23,0.7)', border: '1px solid rgba(30,41,59,0.8)' }}>
            <p className="text-[8px] uppercase tracking-widest text-slate-500 mb-0.5">{label}</p>
            <p className="font-mono text-[11px] font-bold" style={{ color: meta.accent }}>{value}</p>
          </div>
        ))}
      </div>
    </motion.div>
...
export default function ShortestPathComparison() {
  ...
  const nodePos = { 1: [80, 60], 2: [200, 30], 3: [200, 120], 4: [320, 60], 5: [320, 160], 6: [420, 100] }
  ...
  return (
    <div className="flex flex-col gap-4 p-4 sm:p-6">
      ...
      <div className="rounded-2xl border border-slate-700/60 p-4" style={{ background: 'rgba(15,23,42,0.8)' }}>
        <div className="flex flex-wrap gap-4 items-end justify-between">
          <div className="flex flex-wrap gap-4 items-end">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-cyan-400/80 mb-2">Source Node</p>
              <div className="flex gap-1.5">
                {NODES.map(n => (
                  <button key={n} onClick={() => { setSource(n); handleReset() }}
                    className="w-9 h-9 rounded-lg text-xs font-bold border transition-all"
                    style={{
                      background: source === n ? 'rgba(6,182,212,0.15)' : 'rgba(30,41,59,0.5)',
                      borderColor: source === n ? '#06b6d4' : 'rgba(51,65,85,0.8)',
      ...
      {/* Graph preview */}
      <div className="rounded-xl border border-slate-700/40 p-3" style={{ background: 'rgba(15,23,42,0.6)' }}>
        <p className="text-[9px] uppercase tracking-widest text-slate-500 mb-2">Shared Graph (6 nodes, directed, weighted)</p>
        <svg viewBox="0 0 500 210" width="100%" style={{ maxHeight: '140px' }}>
          ...
          {EDGES.map((e, i) => {
            ...
            return (
              <g key={i}>
                <line x1={fx} y1={fy} x2={tx} y2={ty} stroke="rgba(100,116,139,0.5)" strokeWidth="1.5" markerEnd="url(#arrow)" />
                <text x={mx} y={my - 4} textAnchor="middle" fill="#475569" fontSize="9" fontFamily="monospace">{e.weight}</text>
              </g>
            )
          })}
          {Object.entries(nodePos).map(([id, [x, y]]) => (
            <g key={id}>
              <circle cx={x} cy={y} r="14"
                fill={parseInt(id) === source ? 'rgba(6,182,212,0.25)' : 'rgba(30,41,59,0.9)'}
                stroke={parseInt(id) === source ? '#06b6d4' : 'rgba(51,65,85,0.8)'}
                strokeWidth={parseInt(id) === source ? 2 : 1} />
              <text x={x} y={y + 1} textAnchor="middle" dominantBaseline="middle"
                fill={parseInt(id) === source ? '#06b6d4' : '#94a3b8'} fontSize="10" fontWeight="bold" fontFamily="monospace">
                {id}
              </text>
            </g>
          ))}
        ...
        {winner && (
          <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400">Dijkstra is fastest for non-negative weights. Bellman-Ford handles negatives. Floyd-Warshall gives all-pairs shortest paths.</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {Object.keys(ALGO_META).map(key => (
          <AlgoPanel key={key} algoKey={key} source={source} speed={speed}
