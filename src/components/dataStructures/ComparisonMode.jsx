  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 flex flex-col items-center justify-end gap-1 py-4 min-h-[180px]">
        ...
        <AnimatePresence>
          {[...stack].reverse().map((val, i) => (
            <motion.div key={`${val}-${stack.length - i}`}
              ...
              style={{
                background: i === 0 ? 'rgba(6,182,212,0.2)' : 'rgba(30,41,59,0.8)',
                borderColor: i === 0 ? '#06b6d4' : 'rgba(51,65,85,0.6)',
                color: i === 0 ? '#06b6d4' : '#94a3b8',
                boxShadow: i === 0 ? '0 0 10px rgba(6,182,212,0.2)' : 'none',
              }}>
              {val}
              {i === 0 && <span className="ml-2 text-[8px] text-cyan-500">← TOP</span>}
            </motion.div>
          ))}
        </AnimatePresence>
        <div className="w-28 h-0.5 mt-2" style={{ background: 'rgba(51,65,85,0.8)' }} />
      ...
      <div className="space-y-2 pt-2 border-t border-slate-700/40">
        ...
        {lastOp && <p className="text-[10px] text-cyan-400 font-mono text-center">{lastOp}</p>}
      </div>
    </div>
  )
}
    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 flex flex-col justify-center min-h-[180px]">
          ...
          <AnimatePresence>
            ...
            {queue.map((val, i) => (
              <motion.div key={`${val}-${i}`}
                ...
                style={{
                  background: i === 0 ? 'rgba(16,185,129,0.2)' : i === queue.length - 1 ? 'rgba(245,158,11,0.2)' : 'rgba(30,41,59,0.8)',
                  borderColor: i === 0 ? '#10b981' : i === queue.length - 1 ? '#f59e0b' : 'rgba(51,65,85,0.6)',
                  color: i === 0 ? '#10b981' : i === queue.length - 1 ? '#f59e0b' : '#94a3b8',
                }}>
                {val}
                <span className="text-[6px] mt-0.5" style={{ color: 'inherit', opacity: 0.7 }}>
        ...
        <div className="flex gap-3 mt-3 justify-center">
          <div className="flex items-center gap-1"><div className="w-2 h-2 rounded" style={{ background: '#10b981' }} /><span className="text-[9px] text-slate-500">Front</span></div>
          <div className="flex items-center gap-1"><div className="w-2 h-2 rounded" style={{ background: '#f59e0b' }} /><span className="text-[9px] text-slate-500">Rear</span></div>
        </div>
      </div>
      ...
      <div className="space-y-2 pt-2 border-t border-slate-700/40">
        ...
        <div className="flex gap-1.5">
          ...
          <button onClick={peek} disabled={queue.length === 0}
            className="flex-1 py-1.5 rounded-lg text-xs font-bold border border-slate-600 text-slate-300 hover:bg-slate-700 transition disabled:opacity-40">
            Peek
          </button>
        </div>
        {lastOp && <p className="text-[10px] text-emerald-400 font-mono text-center">{lastOp}</p>}
      </div>
    </div>
  )
}
...
function BSTDemo() {
  ...
  function getPositions(values) {
    ...
    // BFS-style layout
    const pos = {}
    const queue = [{ val: root, x: 180, y: 30, minX: 0, maxX: 360, all: sorted }]
      ...
      <div className="flex-1 min-h-[180px]">
        <svg viewBox="0 0 360 200" width="100%" style={{ maxHeight: '180px' }}>
          {edges.map((e, i) => {
            ...
            if (!f || !t) return null
            return <line key={i} x1={f.x} y1={f.y} x2={t.x} y2={t.y} stroke="rgba(51,65,85,0.6)" strokeWidth="1.5" />
          })}
          {Object.entries(pos).map(([val, { x, y }]) => {
            ...
            return (
              <g key={val}>
                <circle cx={x} cy={y} r={isRoot ? 14 : 11}
                  fill={isFound ? 'rgba(16,185,129,0.3)' : isRoot ? 'rgba(167,139,250,0.2)' : 'rgba(30,41,59,0.9)'}
                  stroke={isFound ? '#10b981' : isRoot ? '#a78bfa' : 'rgba(51,65,85,0.8)'}
                  strokeWidth={isFound || isRoot ? 2 : 1}
                  style={{ filter: isFound ? 'drop-shadow(0 0 6px #10b981)' : 'none' }} />
      ...
      <div className="space-y-2 pt-2 border-t border-slate-700/40">
        <div className="flex gap-1.5">
          <input type="number" value={input} onChange={e => setInput(e.target.value)}
            ...
            placeholder="Value…" />
          <button onClick={insert} disabled={!input || nodes.length >= 15}
          ...
          <button onClick={search} disabled={!input}
        ...
        {lastOp && <p className="text-[10px] font-mono text-center" style={{ color: searched !== null && nodes.includes(searched) ? '#10b981' : '#a78bfa' }}>{lastOp}</p>}
      </div>
    </div>
  )
}
...
const DS_META = [
  {
    ...
    ops: [{ op: 'Push', time: 'O(1)' }, { op: 'Pop', time: 'O(1)' }, { op: 'Peek', time: 'O(1)' }, { op: 'Search', time: 'O(n)' }],
  ...
  },
  {
    ...
    ops: [{ op: 'Enqueue', time: 'O(1)' }, { op: 'Dequeue', time: 'O(1)' }, { op: 'Peek', time: 'O(1)' }, { op: 'Search', time: 'O(n)' }],
  ...
  },
  {
    ...
    ops: [{ op: 'Insert', time: 'O(log n)' }, { op: 'Delete', time: 'O(log n)' }, { op: 'Search', time: 'O(log n)' }, { op: 'Traverse', time: 'O(n)' }],
  ...
  return (
    ...
    >
      ...
      <div className="px-4 pb-3">
        <p className="text-[9px] uppercase tracking-widest text-slate-500 mb-1.5">Time Complexity</p>
        <div className="grid grid-cols-2 gap-1">
          {meta.ops.map(({ op, time }) => (
            <div key={op} className="flex items-center justify-between rounded px-2 py-1"
              style={{ background: 'rgba(2,6,23,0.5)', border: '1px solid rgba(30,41,59,0.6)' }}>
              <span className="text-[9px] text-slate-400">{op}</span>
              <span className="font-mono text-[9px] font-bold" style={{ color: meta.accent }}>{time}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="px-4 pb-4">
        <p className="text-[9px] uppercase tracking-widest text-slate-500 mb-1.5">Common Uses</p>
  ...
  return (
    <div className="flex flex-col gap-4 p-4 sm:p-6">
      ...
      <div className="rounded-2xl border border-slate-700/60 p-4" style={{ background: 'rgba(15,23,42,0.8)' }}>
        <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-cyan-400/80 mb-1">Interactive Comparison</p>
        <p className="text-xs text-slate-400">
          Explore Stack, Queue, and Binary Search Tree side by side. Interact with each to see how they behave differently.
        </p>
      </div>

      {/* Key comparison */}
      <div className="rounded-xl border border-slate-700/40 overflow-hidden" style={{ background: 'rgba(15,23,42,0.6)' }}>
        <div className="px-4 py-2 border-b border-slate-700/40">
          <p className="text-[9px] uppercase tracking-widest text-slate-500">At a Glance</p>
        </div>
        <div className="grid grid-cols-3 divide-x divide-slate-700/40">
          {DS_META.map(m => (
            <div key={m.key} className="px-3 py-2.5 space-y-1">
              <p className="text-[10px] font-bold" style={{ color: m.accent }}>{m.label}</p>
              <p className="text-[9px] text-slate-500">{m.subtitle}</p>
              <p className="text-[9px] text-slate-400">Search: <span className="font-mono font-bold text-slate-300">{m.ops.find(o => o.op === 'Search')?.time}</span></p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-xl px-4 py-3 text-xs text-slate-400 border border-slate-700/40"
        ...
        <strong className="text-slate-300">Choosing the right structure:</strong> Use a <span className="text-cyan-400">Stack</span> when order is LIFO (undo, recursion). Use a <span className="text-emerald-400">Queue</span> for FIFO scenarios (BFS, scheduling). Use a <span className="text-purple-400">BST</span> when you need fast ordered search — it beats both for lookups on sorted data at O(log n) vs O(n).
      </div>
    </div>
  )
}